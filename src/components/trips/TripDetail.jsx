...
const EDIT_FIELDS = [
  ...
  { key: 'freight_amount',       label: 'Freight Amount (Rs.)', type: 'number' },
...
export default function TripDetail() {
  ...
  if (!trip) return (
    <div className="p-8 text-center">
      ...
      <button onClick={() => navigate('/trips')} className="mt-4 text-teal-600 text-sm font-medium hover:underline">Back to Trips</button>
    </div>
  )

  const showToast = (msg, ok = true) => { setToast({ msg, ok }); setTimeout(() => setToast(null), 3000) }

  const markInTransit = async () => { await updateTrip(id, { status: 'In Transit' }); showToast('Status updated to In Transit') }
  const markCompleted = async () => { await updateTrip(id, { status: 'Completed', end_date: new Date().toISOString().split('T')[0] }); showToast('Trip marked as Completed') }
  ...
  const handleSave = async () => {
    ...
    } catch (e) { console.error(e); showToast('Failed to save changes', false) }
  ...
  return (
    <div className="p-6 max-w-3xl relative">
      {toast && (
        <div className={'fixed top-5 right-5 z-50 px-4 py-3 rounded-xl shadow-lg text-sm font-medium ' + (toast.ok ? 'bg-green-600 text-white' : 'bg-red-600 text-white')}>
          {toast.msg}
        </div>
      )}

      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate('/trips')} className="flex items-center gap-2 text-gray-500 hover:text-gray-800 text-sm font-medium transition-colors">
          <ArrowLeft size={16} />Back to Trips
        </button>
        <button onClick={startEdit} className="flex items-center gap-2 px-4 py-2 bg-teal-600 text-white text-sm font-semibold rounded-xl hover:bg-teal-700 transition-colors shadow-sm">
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">{trip.from_location} to {trip.to_location}</p>
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            {trip.status === 'Scheduled' && (
              <button onClick={markInTransit} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors shadow-sm">
                <Truck size={15} />Mark In Transit
              </button>
            )}
            {trip.status !== 'Completed' && trip.status !== 'Cancelled' && (
              <button onClick={markCompleted} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-xl hover:bg-green-700 transition-colors shadow-sm">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-gray-100">
              <button onClick={() => setEditing(false)} className="px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">Cancel</button>
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-5 py-2.5 bg-teal-600 text-white text-sm font-semibold rounded-xl hover:bg-teal-700 disabled:opacity-50 transition-colors shadow-sm">
