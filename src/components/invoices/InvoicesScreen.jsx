...
import { Plus, X, Download, Eye, CheckCircle, Clock } from 'lucide-react'
import { useStore } from '../../store/useStore'

// ─── PDF libs loaded from CDN ───────────────────────────────────────────────
...
function InvoiceTemplate({ inv, trip, company, forPdf = false }) {
  ...
  const date   = inv?.created_at ? new Date(inv.created_at).toLocaleDateString('en-IN', { day:'2-digit', month:'long', year:'numeric' }) : '—'
  const due    = inv?.created_at ? new Date(new Date(inv.created_at).getTime() + 30*86400000).toLocaleDateString('en-IN', { day:'2-digit', month:'long', year:'numeric' }) : '—'
    ...
    <div id="invoice-print-area" style={{
      ...
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'32px', paddingBottom:'24px', borderBottom:'3px solid #2E6073'}}>
        <div style={{display:'flex', alignItems:'center', gap:'16px'}}>
          {company?.logo_base64 && (
            <img src={company.logo_base64} alt="logo" style={{height:'70px', width:'auto', objectFit:'contain'}}/>
          )}
          <div>
            <div style={{fontSize:'20px', fontWeight:'800', color:'#2E6073', letterSpacing:'-0.5px'}}>{company?.name || 'Company Name'}</div>
        ...
        <div style={{textAlign:'right'}}>
          ...
          <div style={{marginTop:'8px', fontSize:'13px'}}>
            <span style={{color:'#888'}}>Invoice No: </span><b>{inv?.invoice_number || '—'}</b>
          </div>
          <div style={{fontSize:'13px', marginTop:'4px'}}>
            <span style={{color:'#888'}}>Date: </span><b>{date}</b>
          </div>
          <div style={{fontSize:'13px', marginTop:'4px'}}>
            <span style={{color:'#888'}}>Due Date: </span><b>{due}</b>
          </div>
      ...
      <div style={{display:'flex', justifyContent:'flex-end', marginBottom:'28px'}}>
        <div style={{width:'280px'}}>
          <div style={{display:'flex', justifyContent:'space-between', padding:'7px 14px', fontSize:'12px', color:'#555'}}>
            <span>Sub Total</span><span>₹ {fmt(base)}</span>
          </div>
          <div style={{display:'flex', justifyContent:'space-between', padding:'7px 14px', fontSize:'12px', color:'#555'}}>
            <span>CGST ({gstRate/2}%)</span><span>₹ {fmt(cgst)}</span>
          </div>
          <div style={{display:'flex', justifyContent:'space-between', padding:'7px 14px', fontSize:'12px', color:'#555', borderBottom:'1px solid #e5e7eb'}}>
            <span>SGST ({gstRate/2}%)</span><span>₹ {fmt(sgst)}</span>
          </div>
...
export default function InvoicesScreen() {
  const { invoices, trips, createInvoice, company } = useStore()
  ...
  const downloadPdf = async (inv) => {
    ...
    try {
      ...
      if (!el) { alert('Invoice not rendered yet, try again.'); setDownloading(false); return }
              ...
              <div key={inv.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">
                    {trip?.client_name && <span>{trip.client_name} &middot; </span>}
                    ₹{Number(inv.total_amount).toLocaleString('en-IN')} &middot; GST {inv.gst_rate}%
                  </div>
                  {inv.created_at && (
                    <div className="text-xs text-gray-400 mt-0.5">{new Date(inv.created_at).toLocaleDateString('en-IN', {day:'2-digit', month:'short', year:'numeric'})}</div>
                  )}
                </div>
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <button onClick={async () => { setShowPreview(inv.id); await new Promise(r=>setTimeout(r,400)); downloadPdf(inv) }}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            {!company?.name && (
              <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
                ⚠️ Company profile incomplete. Go to <b>Settings</b> to add your firm name, GST, logo etc. before generating invoices.
              </div>
            )}
            <form onSubmit={submit} className="space-y-3">
              <div>
                ...
                {completedTrips.length === 0 && <p className="text-xs text-gray-400 mt-1">No completed trips available for invoicing.</p>}
              </div>
              <div>
              ...
              {tripId && (
                <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-600 space-y-1">
                  {(() => {
                    ...
                    const base = Number(t?.freight_amount || 0)
                    const tax  = base * gst / 100
                    return <>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-start justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-4">
            ...
            <div className="overflow-x-auto" ref={printRef}>
              <InvoiceTemplate inv={previewInv} trip={previewTrip} company={company} forPdf={true}/>
            </div>
          </div>
        </div>
      )}
