  ...
  persist(
    (set, get) => ({
      ...
      signUp: async (formData) => {
        set({ isLoading: true })
        try {
          const { user, company } = await auth.signUp(formData)
          set({ user, company })
          toast.success('Account created! Welcome to Nextagen Fleet.')
          return { success: true }
        } catch (err) {
          toast.error(err.message)
          return { success: false, error: err.message }
      ...
      signOut: async () => {
        await auth.signOut()
        set({ user: null, company: null, vehicles: [], drivers: [], trips: [], invoices: [] })
      },
      ...
      addVehicle: async (vehicle) => {
        ...
        toast.success(`${vehicle.registration_no} registered!`)
      },
      ...
      createTrip: async (trip) => {
        ...
        const { data, error } = await db.addTrip({
          ...trip,
        ...
        if (error) {
          const msg = error.message || error.details || 'Failed to create trip'
          toast.error(msg)
      ...
      captureEPOD: async (tripId, epodData) => {
        ...
        set((s) => ({
          trips: s.trips.map((t) =>
            t.id === tripId ? { ...t, epod_captured: true, epod_recipient: epodData.recipient } : t
          ),
        }))
        toast.success('ePOD captured! WhatsApp notification sent.')
      },
      ...
      createInvoice: async (tripId, invoiceData) => {
        ...
        await get().updateTrip(tripId, { invoice_raised: true, invoice_number: invoiceNumber })
      ...
      setupRealtime: () => {
        ...
        try {
          const subs = [
            subscribeToVehicles(companyId, () => get().loadVehicles()),
            subscribeToTrips(companyId, () => get().loadTrips()),
          ]
          set({ subscriptions: subs })
        } catch (e) {
          console.warn('setupRealtime skipped:', e.message)
        }
      },
